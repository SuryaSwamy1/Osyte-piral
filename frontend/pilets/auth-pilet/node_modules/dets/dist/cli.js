...
var require_require_directory = __commonJS({
  "node_modules/require-directory/index.js"(exports, module2) {
    ...
    function requireDirectory(m, path, options) {
      ...
      path = !path ? dirname2(m.filename) : resolve3(dirname2(m.filename), path);
...
var require_p_limit = __commonJS({
  "node_modules/p-limit/index.js"(exports, module2) {
    ...
    var pLimit = (concurrency) => {
      ...
      const run = (fn, resolve3, ...args2) => {
        activeCount++;
        const result = pTry(fn, ...args2);
        resolve3(result);
        result.then(next, next);
      };
      const enqueue = (fn, resolve3, ...args2) => {
        if (activeCount < concurrency) {
          run(fn, resolve3, ...args2);
        } else {
          queue.push(run.bind(null, fn, resolve3, ...args2));
        }
      };
      const generator = (fn, ...args2) => new Promise((resolve3) => enqueue(fn, resolve3, ...args2));
...
function includeApi(context, node, interfaceName) {
  if ((0, import_typescript.isInterfaceDeclaration)(node) && node.name.text === interfaceName) {
...
function getJsDocs(checker, node) {
  if ((0, import_typescript2.isMethodDeclaration)(node) || (0, import_typescript2.isMethodSignature)(node) || (0, import_typescript2.isCallSignatureDeclaration)(node)) {
...
var import_typescript2, newLineTags, removedTags;
...
function getTypeRefName(name) {
  if ((0, import_typescript3.isIdentifier)(name)) {
    return name.text;
  } else {
...
function getExportName(name) {
  if (!name) {
    return void 0;
  } else if ((0, import_typescript3.isIdentifier)(name)) {
    return name.text;
  } else if ((0, import_typescript3.isStringLiteral)(name)) {
    return name.text;
  } else {
    return name.text;
  }
...
function getParameterElement(element) {
  return (0, import_typescript3.isBindingElement)(element) ? getParameterName(element.name) : getParameterName(element);
}
function getParameterName(name) {
  if ((0, import_typescript3.isIdentifier)(name)) {
    return name.text;
  } else if ((0, import_typescript3.isObjectBindingPattern)(name)) {
    const content = name.elements.map(getParameterElement).join(", ");
    return `{ ${content} }`;
  } else if ((0, import_typescript3.isArrayBindingPattern)(name)) {
    const content = name.elements.map(getParameterElement).join(", ");
    return `[${content}]`;
  } else {
    return "";
...
var import_typescript3, import_path;
...
function getModule(node) {
  while (node) {
    if ((0, import_typescript4.isModuleDeclaration)(node) && (0, import_typescript4.isSourceFile)(node.parent) && (0, import_typescript4.isStringLiteral)(node.name)) {
...
function getSymbol(checker, node) {
  ...
  if (symbol) {
    return symbol;
  } else if ((0, import_typescript4.isTypeReferenceNode)(node)) {
...
function shouldInclude(node) {
  return (0, import_typescript4.isModuleDeclaration)(node) || (0, import_typescript4.isExportDeclaration)(node) || isNodeExported(node);
}
function isNodeExported(node, alsoTopLevel = false) {
  return (0, import_typescript4.isExportAssignment)(node) || ((0, import_typescript4.getCombinedModifierFlags)(node) & import_typescript4.ModifierFlags.Export) !== 0 || alsoTopLevel && !!node.parent && node.parent.kind === import_typescript4.SyntaxKind.SourceFile;
}
function isPrivate(type) {
  return type.kind === import_typescript4.SyntaxKind.PrivateKeyword;
}
function isStatic(type) {
  return type.kind === import_typescript4.SyntaxKind.StaticKeyword;
}
function isProtected(type) {
  return type.kind === import_typescript4.SyntaxKind.ProtectedKeyword;
}
function isReadonly(type) {
  return type.kind === import_typescript4.SyntaxKind.ReadonlyKeyword;
}
...
function fullyQualifiedName(symbol, delimiter) {
  ...
  } while (symbol && symbol.flags === import_typescript5.SymbolFlags.NamespaceModule && symbol.name !== globalIndicator);
...
function getSymbolName(symbol) {
  if (symbol.flags === import_typescript5.SymbolFlags.EnumMember) {
...
function runAll(context, plugins, type) {
  ...
  plugins.filter((p) => p.type === type).forEach((p) => {
    try {
      p.run(context);
    } catch (ex) {
...
function getName(dir) {
  const location = (0, import_path3.resolve)(dir, "package.json");
...
function writeFile(path, content) {
  const full = (0, import_path3.resolve)(root, path);
  const dir = (0, import_path3.dirname)(full);
...
function runScript(script, cwd) {
  return new Promise((resolve3, reject) => {
    ...
    cp.on("close", (code, signal) => code === 0 ? resolve3() : reject(new Error(signal)));
  });
}
function runCli() {
  writeFile(
    args.out,
    (init_src(), __toCommonJS(src_exports)).generateDeclaration({
      root,
      name: args.name,
...
try {
  ...
  runScript("npm install typescript@^5", (0, import_path3.resolve)(__dirname, "..")).then(runCli);
}
